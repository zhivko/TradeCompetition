We will prepare environment for llm agent to trade.


Role coordinator:
Prepares state of market each minute and pull information from binance and prepare state of the market for all coins.
Coins that we follow are:
BTC, ETH, BNB, XRP, DOGE
it will prepare user prompt with current market state as bellow and we will give agent ACCOUNT INFORMATION & PERFORMANCE.

Agent will analyse the market state and give trade plan estimation if to enter to short or long position. If he enters trade he will set SL and TP, and prepare invalidation plan when the trade needs to be closed.
it will persist data in trade.xml in this element:
...
Here is sample of user prompt that will be feeded each minute to agent.
...
MACD indicators: [-1.124, -1.05, -1.004, -1.036, -1.065, -1.179, -1.415, -1.608, -1.636, -1.672]
...
Role agent:
Prepare a python code for agent that will start trading with 10000 USDT, and it will be feeded with USER_PROMPT.
Agent code will use .env file with deepseek_api_key to prepare client and use it to ask deepseek v3.1 about when to trade and to prepare plan.

agent will persist this plan in XML file trade.xml. agent xml entris.
...
<active_trade>
    ...
    <entry_price>112430.1</entry_price>
    <current_price>113521.5</current_price>
    <liquidation_price>109313.32</liquidation_price>
    <unrealized_pnl>731.24</unrealized_pnl>
    <leverage>25</leverage>
    <exit_plan>
        <profit_target>114607.71</profit_target>
        <stop_loss>111236.89</stop_loss>
        <invalidation_condition>Price closes below 111000 on 3-minute candle</invalidation_condition>
    </exit_plan>
    <confidence>0.88</confidence>
    <risk_usd>747.92</risk_usd>
    <sl_oid>212823854807</sl_oid>
    <tp_oid>212823807244</tp_oid>
    <wait_for_fill>false</wait_for_fill>
    <entry_oid>212823778576</entry_oid>
    <notional_usd>76059.4</notional_usd>
</trade>
</active_trades>
...
<closed_trade>
    ...
    <quantity>0.67</quantity>
    <entry_price>112430.1</entry_price>
    <exit_price>113521.5</current_price>
    <leverage>25</leverage>
    <PNL>
<closed_trade>
</closed_trades>
...
<summary>
    <total_return>93.06%</total_return>
    <available_cash>15581.17</available_cash>
    <current_account_value>19305.76</current_account_value>
    <sharpe_ratio>0.326</sharpe_ratio>
</summary>
</agent>