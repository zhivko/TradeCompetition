...
# Exchange preferences for data fetching
EXCHANGE_PREFERENCES = {
    "BTCUSDT": ["binance", "bybit"],  # Try Binance first, then Bybit for 5m timeframe
    "ETHUSDT": ["binance", "bybit"],
    "BNBUSDT": ["binance", "bybit"],
    "XRPUSDT": ["binance", "bybit"],
    "DOGEUSDT": ["binance", "bybit"],
    "SOLUSDT": ["binance", "bybit"],
    "BTCDOM": ["coinmarketcap"]  # BTC dominance from CoinMarketCap
}

# Global exchange switch - can be set to "bybit", "binance", or "auto"
DEFAULT_EXCHANGE = "binance"  # "auto" uses preferences, "bybit" forces Bybit, "binance" forces Binance

def set_default_exchange(exchange: str) -> None:
    """Set the default exchange to use for data fetching.

    Args:
        exchange: "auto", "bybit", or "binance"
    """
    global DEFAULT_EXCHANGE
    if exchange not in ["auto", "bybit", "binance"]:
        raise ValueError(f"Invalid exchange: {exchange}. Must be 'auto', 'bybit', or 'binance'")
    DEFAULT_EXCHANGE = exchange
    logger.info(f"Default exchange set to: {exchange}")

def get_exchange_preferences_for_symbol(symbol: str) -> list[str]:
    """Get exchange preferences for a symbol based on the current default exchange setting."""
    if DEFAULT_EXCHANGE == "bybit":
        return ["bybit"]
    elif DEFAULT_EXCHANGE == "binance":
        return ["binance"]
    else:  # auto
        return EXCHANGE_PREFERENCES.get(symbol, ["bybit", "binance"])

def get_current_exchange_setting() -> str:
    """Get the current default exchange setting."""
    return DEFAULT_EXCHANGE

# Logger is configured in logging_config.py
...
async def publish_resolution_kline(symbol: str, resolution: str, kline_data: dict, active_websockets: dict = None) -> None:
    try:
        redis = await get_redis_connection()
        ...
        await notify_clients_of_history_update(symbol, resolution, [kline_data], active_websockets)
    except Exception as e:
        logger.error(f"Error publishing resolution kline to Redis: {e}", exc_info=True)

async def notify_clients_of_history_update(symbol: str, resolution: str, klines: list, active_websockets: dict = None) -> None:
    """Notify clients viewing the symbol about new historical data via direct WebSocket.

    This unified function handles both single kline updates and multiple kline updates.
    It filters klines based on client time ranges and tracks last sent timestamps for delta updates.
    """
    try:
        if not active_websockets:
            return

        # Ensure klines is a list (handle single kline case)
        if not isinstance(klines, list):
            klines = [klines]

        # Check each active WebSocket client to see if they are viewing this symbol and resolution
        disconnected_clients = []
        for client_id, client_info in active_websockets.items():
            try:
                websocket = client_info.get('websocket')
                current_symbol = client_info.get('current_symbol')
                current_resolution = client_info.get('current_resolution')
                current_from_ts = client_info.get('from_ts')
                current_to_ts = client_info.get('to_ts')
                ...
                if current_symbol == symbol and current_resolution == resolution:
                    # Filter klines to only include those within the client's requested time range
                    # AND newer than the last timestamp they received
                    filtered_klines = []
                    last_sent_ts = client_info.get('last_sent_timestamp')

                    if current_from_ts is not None and current_to_ts is not None:
                        for kline in klines:
                            kline_ts = kline.get('time')
                            # Check if kline is within time range AND newer than last sent
                            if (current_from_ts <= kline_ts <= current_to_ts and
                                (last_sent_ts is None or kline_ts > last_sent_ts)):
                                filtered_klines.append(kline)
                    else:
                        # If no time range info, send all klines (backward compatibility)
                        filtered_klines = klines

                    # Only send if there are klines to send
                    if filtered_klines:
                        # Send history update notification directly to this client
                        history_update_message = {
                            "type": "history_update",
                            "data": {
                                "symbol": symbol,
                                "resolution": resolution,
                                "ohlcv": filtered_klines,
                                "timestamp": datetime.fromtimestamp(time.time(), timezone.utc).isoformat().replace('+00:00', 'Z')
                            }
                        }

                        await websocket.send_json(history_update_message)
                        logger.debug(f"Sent history update to WebSocket client {client_id} for {symbol} {resolution} with {len(filtered_klines)} klines (delta from {last_sent_ts})")

                        # Update client's last sent timestamp
                        if filtered_klines:
                            latest_ts = max(kline.get('time', 0) for kline in filtered_klines)
                            client_info['last_sent_timestamp'] = latest_ts

            except Exception as e:
                logger.error(f"Error sending history update to WebSocket client {client_id}: {e}")
                disconnected_clients.append(client_id)
                continue
        ...
        for client_id in disconnected_clients:
            if client_id in active_websockets:
                ...
                logger.debug(f"Removed disconnected client {client_id} from active_websockets")

    except Exception as e:
        logger.error(f"Error in notify_clients_of_history_update: {e}", exc_info=True)

async def notify_clients_of_new_trade_via_websocket(symbol: str, exchange: str, trades: list, active_websockets: dict) -> None:
    """Notify clients viewing the symbol about new trade data via direct WebSocket."""
    try:
        if not active_websockets:
            return

        # Check each active WebSocket client to see if they are viewing this symbol
        disconnected_clients = []
        for client_id, client_info in active_websockets.items():
            try:
                websocket = client_info.get('websocket')
                ...
                if current_symbol == symbol:
                    # Send trade notification directly to this client
                    ...
                    await websocket.send_json(trade_update_message)
                    logger.debug(f"Sent trade update to WebSocket client {client_id} for {symbol} on {exchange}")

            except Exception as e:
                logger.error(f"Error sending trade update to WebSocket client {client_id}: {e}")
                disconnected_clients.append(client_id)
                continue
        ...
        logger.error(f"Error in notify_clients_of_new_trade_via_websocket: {e}", exc_info=True)
...
async def notify_clients_of_new_trade(symbol: str, exchange: str, trade: dict) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Error scanning client keys: {scan_error}")
            return
        ...
        for client_key in client_keys:
            try:
                ...
                if client_data.get("symbol") == symbol:
                    ...
                    # Also publish to Redis streams for WebSocket forwarding
                    await redis.xadd("trade_updates", {
                        "data": json.dumps({
                            "symbol": symbol,
                            "exchange": exchange,
                            "trade": trade,
                    ...
                    # WebSocket updates are handled via Redis streams and notification listeners
...
def fetch_open_interest_from_bybit(symbol: str, resolution: str, start_ts: int, end_ts: int) -> list[Dict[str, Any]]:
    """Fetches open interest data from Bybit API."""

    logger.info(f"Fetching open interest for {symbol} {resolution} from {datetime.fromtimestamp(start_ts, timezone.utc)} to {datetime.fromtimestamp(end_ts, timezone.utc)}")
    all_oi_data = []
    ...
    # Import required modules for Bybit API
    try:
        from pybit.unified_trading import HTTP
        session = HTTP(testnet=False)
    except ImportError:
        logger.error("pybit not installed. Cannot fetch from Bybit API.")
        return []

    while current_start < end_ts:
        batch_count += 1
        batch_end = min(current_start + (1000 * timeframe_seconds) - 1, end_ts)
        logger.debug(f"Fetching OI batch #{batch_count} for {symbol} {resolution}: {datetime.fromtimestamp(current_start, timezone.utc)} to {datetime.fromtimestamp(batch_end, timezone.utc)}")

        try:
            # Map resolution to Bybit interval format
            interval_map = {
                "1m": "1",
                "5m": "5",
                "1h": "60",
            ...
            response = session.get_open_interest(
                ...
                endTime=batch_end * 1000,
                limit=1000
            )
        except Exception as e:
            logger.error(f"Bybit API request failed for OI {symbol} {resolution} batch #{batch_count}: {e}")
            break

        if response.get("retCode") != 0:
            logger.error(f"Bybit API error for OI {symbol} {resolution} batch #{batch_count}: {response.get('retMsg', 'Unknown error')} (retCode: {response.get('retCode')})")
            break

        records = response.get("result", {}).get("list", [])
        if not records:
            logger.info(f"No more OI data available from Bybit for {symbol} {resolution} at batch #{batch_count}")
            break

        logger.debug(f"Received {len(records)} OI records from Bybit for {symbol} {resolution} batch #{batch_count}")
        total_records_received += len(records)

        batch_oi = [format_oi_data(record) for record in reversed(records)]
        all_oi_data.extend(batch_oi)

        if not batch_oi:
            logger.warning(f"Batch OI became empty after formatting for {symbol} {resolution} batch #{batch_count}, stopping fetch")
            break

        last_fetched_ts = batch_oi[-1]["time"]
        logger.debug(f"Processed OI batch #{batch_count} for {symbol} {resolution}: {len(batch_oi)} records, last timestamp: {datetime.fromtimestamp(last_fetched_ts, timezone.utc)}")

        if len(records) < 1000 or last_fetched_ts >= batch_end:
            logger.debug(f"Stopping OI batch fetch for {symbol} {resolution}: received {len(records)} records (less than 1000) or reached batch end")
            break
        current_start = last_fetched_ts + timeframe_seconds

    all_oi_data.sort(key=lambda x: x["time"])
    logger.info(f"Completed Bybit OI fetch for {symbol} {resolution}: {batch_count} batches, {total_records_received} total records received, {len(all_oi_data)} OI records processed")
    return all_oi_data

def fetch_open_interest_from_binance(symbol: str, resolution: str, start_ts: int, end_ts: int) -> list[Dict[str, Any]]:
    """Fetches open interest data from Binance API as fallback.
    
    Note: Binance futures_open_interest_hist doesn't support historical date ranges.
    It only returns the most recent open interest data.
    """
    logger.info(f"Fetching current open interest from Binance for {symbol}")
    
    # Import required modules for Binance API
    try:
        from binance.client import Client
        # Use environment variables or default to None for public access
        api_key = os.getenv('BINANCE_API_KEY')
        api_secret = os.getenv('BINANCE_API_SECRET')
        client = Client(api_key, api_secret)
    except ImportError:
        logger.error("python-binance not installed. Cannot fetch from Binance API.")
        return []
    except Exception as e:
    ...
    try:
        # Get current open interest from Binance (this is the only data available from their API)
        ...
        logger.info(f"Fetched current open interest from Binance for {symbol}: {oi_data['openInterest']}")
        return formatted_data
        
    except Exception as e:
        logger.error(f"Error fetching current open interest from Binance for {symbol}: {e}")
        return []
...
def fetch_klines_from_bybit(symbol: str, resolution: str, start_ts: int, end_ts: int) -> list[Dict[str, Any]]:
    ...
    try:
        from pybit.unified_trading import HTTP
        session = HTTP(testnet=False, timeout=3)  # Add 30 second timeout to prevent hanging
    except ImportError:
        logger.error("pybit not installed. Cannot fetch from Bybit API.")
        return []
...
def fetch_klines_from_binance(symbol: str, resolution: str, start_ts: int, end_ts: int) -> list[Dict[str, Any]]:
    """Fetches klines from Binance API using python-binance library with proper pagination."""

    logger.info(f"Fetching klines from Binance for {symbol} {resolution} from {datetime.fromtimestamp(start_ts, timezone.utc)} to {datetime.fromtimestamp(end_ts, timezone.utc)}")
    ...
    # Map resolution to Binance interval format
    interval_map = {
    ...
    interval = interval_map.get(resolution, Client.KLINE_INTERVAL_5MINUTE)
    ...
    while current_start < end_ts:
        batch_count += 1
        # Calculate batch end time (1000 intervals ahead or end_ts, whichever is sooner)
        batch_end = min(current_start + (1000 * timeframe_seconds), end_ts)
        logger.debug(f"Fetching batch #{batch_count} for {symbol} {resolution}: {datetime.fromtimestamp(current_start, timezone.utc)} to {datetime.fromtimestamp(batch_end, timezone.utc)}")

        try:
            # Fetch historical klines using python-binance library
            klines = client.get_historical_klines(
                symbol=symbol,
                interval=interval,
                start_str=int(current_start * 1000),  # Convert to milliseconds
                end_str=int(batch_end * 1000),    # Convert to milliseconds
                limit=1000  # Maximum limit per request
            )
        except Exception as e:
            logger.error(f"Binance API request failed for {symbol} {resolution} batch #{batch_count}: {e}")
            break

        if not klines:
            logger.info(f"No more data available from Binance for {symbol} {resolution} at batch #{batch_count}")
            break

        logger.debug(f"Received {len(klines)} bars from Binance for {symbol} {resolution} batch #{batch_count}")
        total_bars_received += len(klines)

        # Format the klines data
        batch_klines = [format_binance_kline_data(kline) for kline in klines]
        all_klines.extend(batch_klines)
        ...
        # Move to next batch - start from the timestamp after the last fetched kline
        last_fetched_ts = batch_klines[-1]["time"]
        logger.debug(f"Processed batch #{batch_count} for {symbol} {resolution}: {len(batch_klines)} klines, last timestamp: {datetime.fromtimestamp(last_fetched_ts, timezone.utc)}")

        # If we got fewer than 1000 records or reached the batch end, we're done
        if len(klines) < 1000 or last_fetched_ts >= batch_end:
            logger.debug(f"Stopping batch fetch for {symbol} {resolution}: received {len(klines)} bars (less than 1000) or reached batch end")
            break
            
        # Move to next batch - add one interval to avoid duplicate
        current_start = last_fetched_ts + timeframe_seconds

    all_klines.sort(key=lambda x: x["time"])
    logger.info(f"Completed Binance fetch for {symbol} {resolution}: {batch_count} batches, {total_bars_received} total bars received, {len(all_klines)} klines processed")
    return all_klines
...
def format_binance_kline_data(kline: list[Any]) -> Dict[str, Any]:
    """Formats raw Binance kline data into standardized format."""
    return {
        "time": int(kline[0]) // 1000,  # Binance returns milliseconds
        "open": float(kline[1]),
        "high": float(kline[2]),
        "low": float(kline[3]),
        "close": float(kline[4]),
...
def format_oi_data(record: list[Any]) -> Dict[str, Any]:
    """Formats raw Bybit open interest data into standardized format."""
    return {
        "time": int(record[0]) // 1000,
        "open_interest": float(record[1]),
        "symbol": record[2] if len(record) > 2 else ""
    }

def format_binance_oi_data(record: dict) -> Dict[str, Any]:
    """Formats raw Binance open interest data into standardized format."""
    return {
        "time": int(record['timestamp']) // 1000,  # Binance returns milliseconds
...
async def fill_data_gaps(gaps: List[Dict[str, Any]], app=None) -> None:
    ...
    for gap in gaps:
        try:
            ...
            # 📈 ROUTE TO CORRECT DATA SOURCE BASED ON SYMBOL PREFERENCES
            missing_klines = None

            # Get exchange preferences for this symbol based on current default exchange setting
            exchange_preferences = get_exchange_preferences_for_symbol(symbol)
            ...
            for exchange in exchange_preferences:
                try:
                    if exchange == "bybit":
                        ...
                        missing_klines = fetch_klines_from_binance(symbol, resolution, from_ts, to_ts)
                    elif exchange == "coinmarketcap":
                        missing_klines = await fetch_btc_dominance(symbol, resolution, from_ts, to_ts)

                    if missing_klines:
                        logger.info(f"✅ Successfully fetched {len(missing_klines)} klines for {symbol} {resolution} from {exchange}")
                        break
                    else:
                        logger.info(f"Exchange {exchange} returned no data for {symbol} {resolution}, trying next exchange...")

                except Exception as e:
                    logger.warning(f"Failed to fetch from {exchange} for {symbol} {resolution}: {e}")
                    continue
            ...
            if missing_klines:
                ...
                # DISABLED: Notify clients about the newly filled historical data
                # await notify_clients_of_new_history_data(symbol, resolution, missing_klines, app.state.active_websockets if app and hasattr(app.state, 'active_websockets') else None)
            else:
...
async def fill_trade_data_gaps(gaps: List[Dict[str, Any]], app=None) -> None:
    ...
    for gap in gaps:
        try:
            ...
            missing_bars = await fetch_trades_from_ccxt(exchange_id, symbol, from_ts, to_ts, app)
...
async def cache_individual_trades(trades: list[Dict[str, Any]], exchange_name: str, symbol: str, app=None) -> None:
    """Cache individual trades to Redis with TTL."""
    if not trades:
        return

    # logger.info(f"About to cache {len(trades)} individual trades for {symbol} on {exchange_name}")
    ...
    try:
        ...
        # Notify clients about new trades via WebSocket if app is provided
        if app and hasattr(app.state, 'active_websockets'):
            await notify_clients_of_new_trade_via_websocket(symbol, exchange_name, trades, app.state.active_websockets)
        else:
            # Fallback to Redis notifications
            for trade in trades:
...
async def get_individual_trades(exchange_name: str, symbol: str, start_ts: int, end_ts: int) -> list[Dict[str, Any]]:
    """Retrieve individual trades from Redis for aggregation with optimized query."""
    try:
        redis = await get_redis_connection()
        sorted_set_key = f"trades:{exchange_name}:{symbol}"

        # OPTIMIZATION: Add LIMIT to prevent excessive memory usage for large datasets
        # For Binance, limit to 50,000 trades per query to prevent memory issues
        max_trades_limit = 100000
        ...
        trades_data_redis = await redis.zrangebyscore(
            ...
            num=max_trades_limit  # Limit results
        )
        ...
        if len(trades) >= max_trades_limit:
            logger.warning(f"PERF: Hit trade limit of {max_trades_limit} for {exchange_name}:{symbol} - results may be incomplete")
