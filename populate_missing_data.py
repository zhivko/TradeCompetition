...
async def main():
    ...
    print(f"Current exchange setting: {current_exchange}")
    ...
    if len(sys.argv) > 1:
        ...
        if exchange_arg in ["auto", "bybit", "binance"]:
            ...
            print(f"Exchange setting changed to: {exchange_arg}")
        else:
            print(f"Invalid exchange argument: {exchange_arg}. Use 'auto', 'bybit', or 'binance'")

    print("Checking and populating data from January 1, 2021 for all coins...")
    ...
    for coin in coins:
        ...
        if coverage_percentage < 95.0:  # Less than 95% coverage
            print(f"[WARNING] Insufficient data coverage for {coin} ({coverage_percentage:.1f}%) - will refetch complete dataset")
            
            print(f"Fetching data for {coin} from {time.strftime('%Y-%m-%d', time.localtime(start_ts))} to {time.strftime('%Y-%m-%d', time.localtime(end_ts))}")

            # For 5m timeframe, specifically use Binance API for complete refetch
            klines = None
            print(f"[INFO] Attempting to fetch 5m data for {coin} from Binance...")
            ...
            if klines:
                ...
                if filtered_klines:
                    ...
                    print(f"[CLEANUP] Deleted {deleted_keys} individual keys and sorted set for {coin}")

                    await cache_klines(symbol, resolution, filtered_klines)
                    print(f"[SUCCESS] Cached {len(filtered_klines)} klines for {coin}")
                else:
                    print(f"[ERROR] No valid klines data after filtering for {coin} - clearing Redis data and refetching from Bybit as fallback")
                    ...
                    print(f"[REFETCH] Refetching data for {coin} from Bybit as fallback...")
                    ...
                    if fresh_klines:
                        ...
                        if len(fresh_filtered_klines) < len(fresh_klines):
                            print(f"[WARNING] Filtered out {len(fresh_klines) - len(fresh_filtered_klines)} records from fresh Bybit data for {coin}")

                        if fresh_filtered_klines:
                            await cache_klines(symbol, resolution, fresh_filtered_klines)
                            print(f"[SUCCESS] Cached {len(fresh_filtered_klines)} fresh klines for {coin}")
                        else:
                            print(f"[ERROR] Fresh Bybit data also contains no valid klines for {coin}")
                    else:
                        print(f"[ERROR] Failed to refetch data from Bybit for {coin}")
            else:
        ...
        try:
            ...
            print(f"[WARNING] Bybit OI fetch failed for {coin}: {e}")
        ...
        if oi_data:
            ...
            print(f"[WARNING] No open interest data fetched for {coin} from any source")
...
if __name__ == "__main__":
    print("Usage: python populate_missing_data.py [auto|bybit|binance]")
    print("  auto  - Use exchange preferences (default)")
    print("  bybit - Force use of Bybit API only")
    print("  binance - Force use of Binance API only")
    print()
    asyncio.run(main())
