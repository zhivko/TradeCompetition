...
mylog_file_path = LOGS_DIR / 'mylog.log'
...
mylog_handler = FlushingFileHandler(mylog_file_path, encoding="utf-8")
mylog_handler.setLevel(logging.DEBUG)  # File shows DEBUG and above

# Create formatters and add to handlers
formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(filename)s:%(lineno)d:%(funcName)s - %(message)s')
console_handler.setFormatter(formatter)
file_handler.setFormatter(formatter)
mylog_handler.setFormatter(formatter)

# Configure root logger
logging.basicConfig(
    level=logging.DEBUG,  # Root logger level
    handlers=[console_handler, file_handler, mylog_handler]
)
...
# Configure websockets.client logger to WARNING level to reduce WebSocket debug logs
